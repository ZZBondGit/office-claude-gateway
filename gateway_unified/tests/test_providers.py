...
class TestAutoProvider:
    ...
    def test_sk_api_prefix_routes_to_minimax_payg(self, monkeypatch):
        captured = {}
        ...
        response = client.post("/v1/messages", headers={"x-api-key": "sk-api-autotest"}, json=_build_base_body())
        assert response.status_code == 200
        assert captured["url"] == "https://api.minimaxi.com/anthropic/v1/messages"
        assert captured["auth"] == "Bearer sk-api-autotest"

    def test_sk_cp_prefix_routes_to_minimax_codingplan(self, monkeypatch):
        captured = {}
        ...
        body = _build_base_body()
        body["model"] = "claude-sonnet-4-5"
        response = client.post("/v1/messages", headers={"x-api-key": "sk-cp-autotest"}, json=body)
        assert response.status_code == 200
        assert captured["url"] == "https://api.minimaxi.com/anthropic/v1/messages"
        assert captured["model"] == "MiniMax-M2.5"
...
class TestSharedEndpoints:
    ...
    def test_models(self, monkeypatch):
        ...
        # 严格客户端通常优先读取前几项，前 3 项必须稳定暴露两档 versioned Claude 模型
        assert ids[:2] == [
            "claude-opus-4-7",
            "claude-sonnet-5",
        ]
        ...
        assert data["first_id"] == "claude-opus-4-7"
    ...
    def test_models_dedup_when_alias_collide(self, monkeypatch):
        ...
        module = _reload_main(monkeypatch, {
            "ACTIVE_PROVIDER": "deepseek",
            "DEEPSEEK_API_KEY": "sk-test",
            "ALIAS_OPUS": "claude-opus-4-7",
            "ALIAS_SONNET": "claude-sonnet-5",
        })
        client = TestClient(module.app)
        response = client.get("/v1/models")
        assert response.status_code == 200

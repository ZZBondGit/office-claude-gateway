...
app = FastAPI(
    title=f"Claude Office Gateway -> {provider.name.title()} (Unified)",
    version="2.0.0",
)
    ...
    *,
    ...
    for tool_use in tool_uses:
        ...
        if not query:
            ...
            _logger.info(
                "[gateway auto_web_search] empty_query_fallback tool_use_id=%s fallback_query=%r",
                tool_use_id,
                query,
            )
...
async def _post_upstream_json_with_retry(
    *,
    upstream_url: str,
    headers: Dict[str, str],
    body: Dict[str, Any],
    ...
    """POST to upstream with retry on 5xx/429/connect errors.

    Streamed requests bypass this helper (you can't safely retry a stream
    once it has started). Use this only for the non-streaming path and the
    web-search auto-execute follow-up calls.
    """
    async def _factory(client: httpx.AsyncClient) -> httpx.Response:
        return await client.post(upstream_url, headers=headers, json=body, timeout=timeout)
...
async def create_message(req: Request):
    ...
    _logger.info("[gateway route] kind=%s provider=%s", route_kind, provider.name)
    ...
    _logger.info(
        ...
        "fp=%s model=%s stream=%s max_tokens=%s "
        "top_keys=%s messages=%s tools=%s "
        "system_present=%s metadata_present=%s "
        "role_seq=%s last_message=%s "
        "block_type_counts=%s",
        req_fp,
        req_summary['model'],
        req_summary['stream'],
        req_summary['max_tokens'],
        req_summary['top_keys'],
        req_summary['messages_count'],
        req_summary['tools_count'],
        req_summary['system_present'],
        req_summary['metadata_present'],
        req_summary['role_seq'],
        req_summary['last_message'],
        req_summary['block_type_counts'],
    )
    ...
    if dropped or removed_fields:
        _logger.info("[gateway sanitize] dropped=%s removed_fields=%s", dropped, removed_fields)
    ...
    try:
        if bool(body.get("stream", False)):
            ...
            if upstream_stream.status_code >= 400:
                ...
                _logger.error("[gateway upstream error] status=%s body=%s", upstream_stream.status_code, err)
            ...
            async def event_stream():
                ...
                try:
                    ...
                    async for raw_line in upstream_stream.aiter_lines():
                        ...
                        if raw_line == "":
                            chunks, pending_event_name, parsed_event = await process_sse_frame(
                                frame_buffer,
                                seen_block_starts,
                                block_kind_by_index,
                            ...
                            if parsed_event is not None:
                                stream_usage = extract_usage_from_sse_event(
                                    str(parsed_event.get("type", "")), parsed_event
                                )
                                if stream_usage is not None:
                                    _logger.info(
                                        "[gateway usage] stream route_kind=%s usage=%s",
                                        route_kind,
                                        format_usage(stream_usage),
                                    )
                            for chunk in chunks:
                                chunk_count += 1
                                yield chunk
                    ...
                    if frame_buffer:
                        chunks, _, parsed_event = await process_sse_frame(
                            frame_buffer,
                            seen_block_starts,
                            block_kind_by_index,
                            tool_partial_json_by_index,
                    ...
                    _logger.exception("[gateway stream error] %s: %s", type(exc).__name__, exc)
                    ...
                    await upstream_stream.aclose()
                    await stream_client.aclose()
                    _logger.info("[gateway stream closed] chunks=%s", chunk_count)
        ...
        # 非流式模式（带 retry：5xx/429/网络错误自动重试3次，指数退避）
        upstream = await _post_upstream_json_with_retry(
            upstream_url=upstream_url, headers=headers, body=body, timeout=60.0
        )
    except httpx.HTTPError as exc:
        err_type = type(exc).__name__
        err_msg = str(exc)
        _logger.error("[gateway upstream http error] type=%s message=%s", err_type, err_msg)
        raise HTTPException(
            status_code=502,
    ...
    if upstream.status_code >= 400:
        ...
        _logger.error("[gateway upstream error] status=%s body=%s", upstream.status_code, upstream.text[:500])
    ...
    # Record token usage for the primary (non-streaming) response.
    initial_usage = extract_usage_from_message(payload) if isinstance(payload, dict) else None
    if initial_usage:
        _logger.info(
            "[gateway usage] route_kind=%s usage=%s",
            route_kind,
            format_usage(initial_usage),
        )
    ...
    if (
        ...
        if auto_tool_uses and payload.get("stop_reason") != "tool_use":
            _logger.info(
                "[gateway auto_web_search] pseudo_tool_call_end_turn stop_reason=%s",
    ...
    if auto_tool_uses:
        ...
        if tool_uses:
            ...
            for round_idx in range(max(1, AUTO_WEB_SEARCH_MAX_ROUNDS)):
                ...
                try:
                    upstream2 = await _post_upstream_json_with_retry(
                ...
                if upstream2.status_code >= 400:
                    ...
                    _logger.error("[gateway upstream error] status=%s body=%s", upstream2.status_code, upstream2.text[:500])
                ...
                # Record token usage for the followup round (auto web_search loop).
                followup_usage = extract_usage_from_message(payload) if isinstance(payload, dict) else None
                if followup_usage:
                    _logger.info(
                        "[gateway usage] route_kind=%s round=%s usage=%s",
                        route_kind,
                        round_idx + 1,
                        format_usage(followup_usage),
                    )
                ...
                if payload.get("stop_reason") != "tool_use":
                    _logger.info(
                        "[gateway auto_web_search] pseudo_tool_call_followup round=%s stop_reason=%s",
                        round_idx + 1,
                        payload.get("stop_reason"),
                    )
                if has_xml_tool_call:
                    _logger.info(
                        "[gateway auto_web_search] xml_tool_call_followup round=%s",
                        round_idx + 1,
                    )
...
def cli():
    ...
    parser.add_argument(
        "--lan",
        action="store_true",
        help="监听 0.0.0.0，允许局域网内其他设备通过 http://<本机IP>:<port> 访问。"
        "默认仅本机 (127.0.0.1)。开启后请确保防火墙放行端口，"
        "且不要在 .env 里配置上游 API key（否则局域网内任何人都能借用）。",
    )
    ...
    # --lan 开关：绑定 0.0.0.0 允许局域网访问；否则保持本机 127.0.0.1

...
class ProviderConfig:
    ...
    def __init__(self, name: str, env_prefix: str):
        ...
        # 别名配置（Office 插件发送简洁名，同时兼容带前缀的长名）
        self.alias_opus = os.getenv("ALIAS_OPUS", "opus").strip()
        self.alias_opus_versioned = os.getenv("ALIAS_OPUS_VERSIONED", "claude-opus-4-7").strip()
        self.alias_sonnet = os.getenv("ALIAS_SONNET", "sonnet").strip()
        self.alias_sonnet_versioned = os.getenv("ALIAS_SONNET_VERSIONED", "claude-sonnet-5").strip()
        ...
        # 默认 2 档（Haiku 合并到 Sonnet）。子类可设为 3 让 Haiku 独立映射。
        self.tier_count: int = 2
    ...
    def route_model(self, model_id: str, route_kind: str = "") -> str:
        ...
        三档模式（MiniMax）：Opus→primary, Sonnet→mid, Haiku→fast（各自独立）
        两档模式（DeepSeek/Kimi/MiMo）：Opus→primary, Sonnet/Haiku→mid（fast 被合并）
        """
        value = (model_id or "").strip()
        if not value:
            return self.model_primary

        # fast_compat：3 档时是 model_fast 自身；2 档时被 mid 吞掉（保持向后兼容）
        fast_compat = self.model_fast if self.tier_count == 3 else (self.model_mid or self.model_fast)
        ...
        if result is not None:
            if result != value:
                _logger.info("[gateway model] mapped %r -> %r", value, result)
            return result
        ...
        for prefix, tier in self._MODEL_PREFIX_TIERS:
            if lower.startswith(prefix):
                ...
                _logger.info("[gateway model] prefix-matched %r (%s*) -> %r", value, prefix, result)
                return result

        # 未知模型 → primary（保守回退）
        _logger.info("[gateway model] unknown model %r, fallback -> %r", value, self.model_primary)
        return self.model_primary
...
class MiMoProvider(ProviderConfig):
    ...
    def __init__(self):
        ...
        if self.tp_region_default not in self.tp_base_urls:
            _logger.warning("[gateway startup] invalid MIMO_TP_REGION=%r; fallback=cn", self.tp_region_default)
...
class MiniMaxProvider(ProviderConfig):
    ...
    def __init__(self):
        ...
        if self.region_default not in self.base_urls:
            _logger.warning("[gateway startup] invalid MINIMAX_REGION=%r; fallback=cn", self.region_default)
            self.region_default = "cn"

        # MiniMax 支持独立 Haiku 档（fast = highspeed），3 档模式。
        self.tier_count = 3
...
class AutoProvider(ProviderConfig):
    ...
    def __init__(self):
        ...
        _logger.info(
            ...
            "deepseek_base=%s kimi_coding=%s kimi_payg=%s mimo_payg=%s",
            _mask_url(self._deepseek.base_url),
            _mask_url(self._kimi.base_url_coding),
            _mask_url(self._kimi.base_url_payg),
            _mask_url(self._mimo.base_url_payg),
        )
    ...
    def _detect_provider_and_key(self, req: Request) -> Tuple[ProviderConfig, str]:
        ...
        for provider in [self._deepseek, self._kimi, self._mimo, self._minimax]:
            if provider.api_key:
                _logger.info(
                    "[gateway auto] env key found for %s, locking all traffic to this provider. "
        ...
        # sk-api-* / sk-cp-* → MiniMax（PAYG / Coding Plan 双计费）
        if lower.startswith("sk-api-") or lower.startswith("sk-cp-"):
            return self._minimax, token
        ...
        if lower.startswith("sk-"):
            _logger.info("[gateway auto] ambiguous sk-* key, defaulting to MiMo PAYG. "
                  "Use sk-kimi-* for Kimi, sk-mimo-* for MiMo, sk-api-* or sk-cp-* for MiniMax, "
                  "dk-* for DeepSeek.")
            return self._mimo, token
    ...
    def resolve_upstream_url(self, req: Request) -> Tuple[str, str, str]:
        ...
        _logger.info("[gateway auto] detected provider=%s", detected_provider.name)
        ...
        if detected_provider is self._minimax:
            # 复用 MiniMaxProvider 的完整路由逻辑（含 region 覆写头 + PAYG/Coding Plan 双计费）
            region = self._minimax._resolve_region(req)
            base_url = self._minimax.base_urls[region]
            if token.startswith("sk-api-"):
                return token, f"{base_url}/v1/messages", f"minimax:payg:{region}"
            if token.startswith("sk-cp-"):
                return token, f"{base_url}/v1/messages", f"minimax:codingplan:{region}"
            raise HTTPException(status_code=500, detail="Internal: MiniMax key prefix unrecognized")
...
def load_provider() -> ProviderConfig:
    ...
    if active != "auto":
        _logger.info(
            "[gateway startup] provider=%s model_primary=%s model_mid=%s model_fast=%s",
            provider.name,

...
async def process_sse_frame(
...
) -> Tuple[List[bytes], str | None, Optional[Dict[str, Any]]]:
    """
    处理一个 SSE 帧，返回 (输出字节列表, 更新后的 pending_event_name, 可选的 parsed event_data)。
    ...
    返回的 parsed_event_data 是当帧的 JSON data（dict）或 None。仅当前帧有完整
    JSON 时返回，供调用方提取 usage / stop_reason 等元数据。返回的 data 已经被透传
    yield 给客户端，调用方无需再单独转发。
    """
    output_chunks: List[bytes] = []
    chunk_count = 0

    if not frame_lines:
        return output_chunks, pending_event_name, None
    ...
    if event_name and not data_lines:
        return output_chunks, event_name, None
    ...
    try:
        ...
        _logger.warning("[gateway malformed sse data] %s: %s; payload_len=%s", type(parse_exc).__name__, parse_exc, len(payload))
    ...
    if (
        ...
        _logger.info(
            "[gateway sse shim] synthetic_start=1 trigger_event=%s index=%s synthetic_type=%s",
            event_type,
            event_index,
            synthetic_block.get("type"),
        )
    ...
    if (
        ...
        if isinstance(delta, dict) and delta.get("type") == "input_json_delta":
            ...
            if isinstance(partial_json, str):
                ...
                _logger.info(
                    "[gateway sse shim] buffered_tool_input_delta=1 index=%s chunk_len=%s total_len=%s",
                    event_index,
                    len(partial_json),
                    len(tool_partial_json_by_index[event_index]),
                )
                return output_chunks, pending_event_name, None
    ...
    if (
        ...
        _logger.info(
            "[gateway sse shim] emitted_normalized_tool_input_delta=1 index=%s reason=%s normalized_len=%s",
            event_index,
            normalize_reason,
            len(normalized_json),
        )
    ...
    output_chunks.append(emit_frame(event_name, payload))
    return output_chunks, pending_event_name, parsed

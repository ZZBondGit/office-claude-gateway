...
_logger = get_logger("request_log")
...
class RequestLogMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        # The middleware preserves the existing log line shapes so downstream
        # log scrapers keep working. We tag with logger name 'request_log'.
        _logger.info("%s %s", method, path)
        _logger.info(
            "headers: origin=%s auth=%s x-api-key=%s api-key=%s "
            "ct=%s anthropic-version=%s anthropic-beta=%s "
            "acr-method=%s acr-headers=%s%s",
            origin,
            auth_flag,
            key_flag,
            api_key_flag,
        ...
        _logger.info("body: %s", body_preview)
        _logger.info(
            "response: %s ct=%s latency_ms=%s captured_bytes=%s body_truncated=%s",
            status_code,
            content_type,
            latency_ms,
            captured_bytes,
        ...
        if seen_more_body:
            _logger.info("note: request body was chunked (more_body=True seen)")

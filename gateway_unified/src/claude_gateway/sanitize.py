...
def sanitize_request_body(
    ...
    if metadata_present:
        ...
        _logger.info("[gateway metadata] present=yes mode=%s %s", mode, summary)
    else:
        mode = "passthrough_enabled" if passthrough_metadata else "removed"
        _logger.info("[gateway metadata] present=no mode=%s", mode)
    ...
    _logger.info(
        "[gateway compat] "
        "probe_kind=%s raw_max_tokens=%s effective_max_tokens=%s",
        probe_kind,
        raw_max_tokens,
